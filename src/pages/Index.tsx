...
const Index = () => {
  ...
  useEffect(() => {
    ...
    Object.keys(answers).forEach(key => {
      ...
      const question = questions.find(q => q.id === questionId);
      if (question && answers[key]) {
        completedSections.add(question.sectionType);
      }
    });
    ...
    setChecklistItems(prevItems =>
      prevItems.map(item => ({
        ...item,
        isCompleted: completedSections.has(item.type)
      }))
    );
  }, [answers]);
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-1 space-y-6">
            ...
            />
            <ComplianceInfo
              improvementAreas={complianceInfo.improvementAreas}
              missingClauses={complianceInfo.missingClauses}
              potentialRisks={complianceInfo.potentialRisks}
            />
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <ContractPreview
              ...
              onComplianceUpdate={setComplianceInfo}
              onContractGenerated={setContractContent}
            />
          </div>
        </div>
      </div>
